...
func main() {
	...
	if *prefix != "" {
		...
		if *nWorkers == 1 {
			err := genereKeyPairPattern(*prefix)
			if err != nil {
				log.Fatal(err)
			}
		} else {
			err := genereKeyPairPatternParallel(*prefix)
			if err != nil {
				log.Fatal(err)
			}
		}
...
func genereKeyPairPattern(prefix string) error {
	var kp *keypair.Full
	var err error
...
func genereKeyPairPatternParallel(prefix string) error {
